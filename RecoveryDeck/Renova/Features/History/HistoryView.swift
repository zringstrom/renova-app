    ...
    private var band: some View {
        HStack(alignment: .top) {
            Text("HISTORY")
                ...
                .tracking(0.5)
            Spacer()
            Text("\(viewModel.historyDays().count) DAYS LOGGED")
    ...
    private func dayRow(_ day: DayRecord) -> some View {
        HStack(spacing: 12) {
            ...
            if let measurement = day.measurement {
                if let rmssd = measurement.rmssdMs {
                    Text("\(rmssd, specifier: "%.0f") ms")
                        .font(.system(size: 11.5, design: .monospaced))
                        .foregroundStyle(CGTheme.inkDim)
                }
                RoundedRectangle(cornerRadius: 2).fill(CGTheme.accent2).frame(width: 8, height: 8)
            } else {
                Text("PARTIAL")
